...
@RequestMapping(
    ...
    try {
               ...
               String solrSearchUrl = "http://localhost:8983/solr/clelog/query?q="+finalQueryString+"&wt=json&indent=true&rows=500";
...
@RequestMapping(method = RequestMethod.POST,consumes = MediaType.APPLICATION_JSON_VALUE,value = "/UpdateLogInfo")
	public String updateLogInformation(@RequestBody LogInformation loginfo)
	{
		System.out.println("Inside UpdateLogInfor Method");
		System.out.println("Status="+loginfo.getStatus().get(0));
		
		String sql="UPDATE CLELOG SET status ="+loginfo.getStatus().get(0)+" WHERE logid ="+loginfo.getLogid().get(0);
		...
		System.out.println("***Updated the hbase********");
		return "Success";
	}
